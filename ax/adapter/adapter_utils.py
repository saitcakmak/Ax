...
def extract_objective_weight_matrix(
    ...
    """Extract a 2D weight matrix for multi-objective optimization with
    scalarized sub-objectives.

    Returns a matrix of shape ``(n_objectives x n_outcomes)`` where each row
    represents one MOO objective as a linear combination of model outputs.
    Signs encode direction (positive = maximize, negative = minimize).

    Returns ``None`` if the objective is not a ``MultiObjective`` with at least
    one ``ScalarizedObjective`` child (i.e., the standard case that doesn't
    need a weight matrix).

    Args:
        objective: Objective to extract the weight matrix from.
        outcomes: n-length list of names of metrics.

    Returns:
        ``(n_objectives x n_outcomes)`` array, or ``None``.
    """
    if not isinstance(objective, MultiObjective):
        return None
    if not any(isinstance(o, ScalarizedObjective) for o in objective.objectives):
        return None

    n_objectives = len(objective.objectives)
    n_outcomes = len(outcomes)
    weight_matrix = np.zeros((n_objectives, n_outcomes))
    for i, obj in enumerate(objective.objectives):
        s = -1.0 if obj.minimize else 1.0
        if isinstance(obj, ScalarizedObjective):
            for metric, weight in obj.metric_weights:
                weight_matrix[i, outcomes.index(metric.signature)] = weight * s
        else:
            weight_matrix[i, outcomes.index(obj.metric.signature)] = s
...
def feasible_hypervolume(
    ...
    if has_scalarized:
        ...
        n_sub_objs = len(objective.objectives)
        # Collect all component metric values and compute scalarized values
        f_cols = []
        obj_thresholds_list = []
        obj_weights_list = []
